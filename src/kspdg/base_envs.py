...
class KSPDGBaseEnv(ABC, gym.Env):
    ...
    def __init__(self, debug: bool = False) -> None:
        ...
        self.logger = create_logger(
            __name__, stream_log_level=logging.DEBUG if debug else logging.INFO
        )
        ...
        if hasattr(self, "episode_termination_thread"):
            if self.episode_termination_thread.is_alive():
                raise ConnectionError(
                    "episode_termination_thread is already running."
                    + " Close and join episode_termination_thread before restarting"
                )

        self.episode_termination_thread = Thread(
            target=self.enforce_episode_termination
        )
