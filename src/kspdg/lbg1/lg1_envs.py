...
class LBG1_LG1_ParentEnv(LadyBanditGuardGroup1Env):
    ...
    def zeroout_bandit_guard_velocity(self):
        ...
        while True:
            ...
            vel_vesB_vesG__lhgntw = self.vesBandit.velocity(
                self.vesGuard.orbital_reference_frame
            )
            spd_vesB_vesG = np.linalg.norm(vel_vesB_vesG__lhgntw)
            self.logger.debug(
                "Zeroing: Bandit-Guard relative speed = {}".format(spd_vesB_vesG)
            )

            # check zero-ing vel breakout conditions to move to next step
            if (
                (spd_vesB_vesG < self.min_speed_thresh)
                or time.time() - loop_start_time > self.loop_timeout
                or self.stop_bot_thread
            ):
                break
            ...
            if self.vesGuard.auto_pilot.error > self.pointing_thresh:
                ...
                self.logger.debug(
                    "Zeroing-Reorientation: Pointing error = {}".format(
                        self.vesGuard.auto_pilot.error
                    )
                )
                continue
    ...
    def direct_burn_guard_toward_bandit(self):
        ...
        pos_vesB_vesG__lhgntw = self.vesBandit.position(
            self.vesGuard.orbital_reference_frame
        )
        ...
        while True:
            ...
            spd_vesB_vesG = np.linalg.norm(vel_vesB_vesG__lhgntw)
            self.logger.debug(
                "DirectBurn: Bandit-Guard relative speed = {}".format(spd_vesB_vesG)
            )
            ...
            ang_vel_pos_rad = U.angle_between(
                -np.array(vel_vesB_vesG__lhgntw), np.array(pos_vesB_vesG__lhgntw)
            )
            ...
            if (
                spd_vesB_vesG > self.max_speed_thresh
                or loop_time > self.loop_timeout
                or self.stop_bot_thread
                or (
                    ang_vel_pos_rad > self.ang_vel_pos_thresh
                    and loop_time > self.min_burn_time
                )
            ):
                break
    ...
    def coast_until_pos_vel_misalign(self):
        ...
        while True:
            ...
            self.logger.debug(
                "Coast: Bandit-Guard vel-pos vector angle = {}".format(ang_vel_pos_rad)
            )

            # check for coast breakout condition to repeat process
            if ang_vel_pos_rad > self.ang_vel_pos_thresh or self.stop_bot_thread:
