...
class EvasionEnvV20220705(gym.Env):
    ...
    def pursue(self):
        ...
        while not self.stop_pursue_thread:
            ...
            self.vesPursue.control.right = (
                p_vesE_vesP__lhpbody[0] / max_axis_parallel_dist
            )
            self.vesPursue.control.forward = (
                p_vesE_vesP__lhpbody[1] / max_axis_parallel_dist
            )
            self.vesPursue.control.up = (
                -p_vesE_vesP__lhpbody[2] / max_axis_parallel_dist
            )
