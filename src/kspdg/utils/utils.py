...
def estimate_capture_dv(p0_prs, v0_prs, p0_evd, v0_evd, tof):
    ...
    pf_e_cb__rhcbci, vf_e_cb__rhcbci = propagate_orbit_tof(
        p0__rhcbci=p0_e_cb__rhcbci, v0__rhcbci=v0_e_cb__rhcbci, time_of_flight=tof
    )
    ...
    v0trans_p_cb__rhcbci, vftrans_p_cb__rhcbci = solve_lambert(
        p0__rhcbci=p0_p_cb__rhcbci, pf__rhcbci=pf_e_cb__rhcbci, time_of_flight=tof
    )
...
def BROKEN_get_rcs_net_directional_properties(vessel, burn_vec__rhbody):
    ...
    dir_thrust_props = [
        BROKEN_get_thruster_directional_properties(t, burn_vec__rhbody)
        for t in thrusters
    ]
    ...
    net_specific_impulse = net_max_effective_thrust / (
        net_max_effective_fuel_consumption * CONST.G0
    )
    return (
        net_max_effective_thrust,
        net_max_effective_fuel_consumption,
        net_specific_impulse,
    )
...
def BROKEN_get_thruster_directional_properties(thruster, burn_vec__rhbody):
    ...
    max_effective_thrust = thruster.part.rcs.max_thrust * max(
        0.0, np.dot(bv__lhbody, tv__lhbody)
    )

    # determine maximum effective fuel consumption
    max_effective_fuel_consumption = max_effective_thrust / (
        CONST.G0 * thruster.part.rcs.specific_impulse
    )
